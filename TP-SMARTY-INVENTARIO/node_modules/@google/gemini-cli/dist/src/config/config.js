...
export async function loadHierarchicalGeminiMemory(currentWorkingDirectory, includeDirectoriesToReadGemini = [], debugMode, fileService, settings, extensions, folderTrust, memoryImportFormat = 'tree', fileFilteringOptions) {
    ...
    return loadServerHierarchicalMemory(effectiveCwd, includeDirectoriesToReadGemini, debugMode, fileService, extensions, folderTrust, memoryImportFormat, fileFilteringOptions, settings.context?.discoveryMaxDirs);
}
/**
...
export async function loadCliConfig(settings, allExtensions, sessionId, argv, cwd = process.cwd()) {
    ...
    const { memoryContent, fileCount, filePaths } = await loadHierarchicalGeminiMemory(cwd, settings.context?.loadMemoryFromIncludeDirectories
        ? includeDirectories
        : [], debugMode, fileService, settings, allExtensions, trustedFolder, memoryImportFormat, fileFiltering);
    ...
    const excludeTools = mergeExcludeTools(settings, allExtensions, extraExcludes.length > 0 ? extraExcludes : undefined);
