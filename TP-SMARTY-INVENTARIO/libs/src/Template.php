...
#[\AllowDynamicProperties] class Template extends TemplateBase
{
    ...
    private $inheritance;
    public function __construct($template_resource, Smarty $smarty, ?\Smarty\Data $_parent = null, $_cache_id = null, $_compile_id = null, $_caching = null, $_isConfig = false)
    {
        $this->smarty = $smarty;
    ...
    } private function render($no_output_filter = true, $display = null)
    {
        if ($this->smarty->debugging) {
            ...
            throw new Exception("Unable to load '{$this->getSource()->type}:{$this->getSource()->name}'" . ($this->_isSubTpl() ? " in '{$this->parent->template_resource}'" : ''));
            ...
            if ($this->caching && $this->smarty->cache_modified_check) {
                $this->smarty->cacheModifiedCheck($this->getCached(), $this, isset($content) ? $content : ob_get_clean());
            } else {
                if ((!$this->caching || $this->getCached()->getNocacheCode() || $this->getSource()->handler->recompiled) && !$no_output_filter) {
            ...
            } if (!$no_output_filter && (!$this->caching || $this->getCached()->getNocacheCode() || $this->getSource()->handler->recompiled)) {
                return $this->smarty->runOutputFilters(ob_get_clean(), $this);
            } return null;
        }
    } public function renderSubTemplate($template_name, $cache_id, $compile_id, $caching, $cache_lifetime, array $extra_vars = [], ?int $scope = null, ?string $currentDir = null)
    {
        ...
        $tpl->inheritance = $this->getInheritance();
        if ($scope) {
    ...
    } public function getCached($forceNew = false): Cached
    {
        if ($forceNew || !isset($this->cached)) {
            ...
            $this->cached = new Cached($this->source, $cacheResource, $this->compile_id, $this->cache_id);
    ...
    } public function mustCompile(): bool
    {
        ...
        } return $this->smarty->force_compile || $this->getSource()->handler->recompiled || !$this->getCompiled()->exists || ($this->compile_check && $this->getCompiled()->getTimeStamp() < $this->getSource()->getTimeStamp());
