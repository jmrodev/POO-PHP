...
{
    public const SMARTY_VERSION = '5.5.2';
    ...
    } public function setPluginsDir($plugins_dir)
    {
        ...
        $this->extensions = array_filter($this->extensions, function ($extension) { return !($extension instanceof DefaultExtension); });
    ...
    } private function generateUniqueTemplateId($resource_name, $cache_id = null, $compile_id = null, $caching = null): string
    {
    ...
    } public function clearCache($template_name, $cache_id = null, $compile_id = null, $exp_time = null)
    {
    ...
    } protected function compileAll($extension, $force_compile, $time_limit, $max_errors, $isConfig = false)
    {
        ...
        foreach ($sourceDir as $_dir) {
            ...
            foreach ($_dir_2 as $_fileinfo) {
                ...
                try {
                    ...
                    $_tpl->setSource($isConfig ? \Smarty\Template\Config::load($_tpl) : \Smarty\Template\Source::load($_tpl));
                    if ($_tpl->mustCompile()) {
    ...
    } public function cacheModifiedCheck(Template\Cached $cached, Template $_template, $content)
    {
        ...
        if ($_isCached && $cached->timestamp <= strtotime($_last_modified_date)) {
            switch (PHP_SAPI) {
                case 'cgi': case 'cgi-fcgi': case 'fpm-fcgi': header('Status: 304 Not Modified');
                    break;
                case 'cli': if (!empty($_SERVER['SMARTY_PHPUNIT_DISABLE_HEADERS'])) {
                    $_SERVER['SMARTY_PHPUNIT_HEADERS'][] = '304 Not Modified';
                } break;
                default: if (!empty($_SERVER['SMARTY_PHPUNIT_DISABLE_HEADERS'])) {
                    $_SERVER['SMARTY_PHPUNIT_HEADERS'][] = '304 Not Modified';
                } else {
            ...
            switch (PHP_SAPI) {
                ...
                default: header('Last-Modified: ' . gmdate('D, d M Y H:i:s', $cached->timestamp) . ' GMT');
                    break;
            } echo $content;
        }
    ...
    public function getRuntime(string $type)
    {
        if (isset($this->runtimes[$type])) {
            ...
            case 'Capture': return $this->runtimes[$type] = new CaptureRuntime();
            case 'Foreach': return $this->runtimes[$type] = new ForeachRuntime();
            case 'Inheritance': return $this->runtimes[$type] = new InheritanceRuntime();
            case 'TplFunction': return $this->runtimes[$type] = new TplFunctionRuntime();
            case 'DefaultPluginHandler': return $this->runtimes[$type] = new DefaultPluginHandlerRuntime($this->getDefaultPluginHandlerFunc());
    ...
    } public function registerFilter($type, $callback, $name = null)
    {
        ...
        if (!is_callable($callback)) {
            ...
            case 'variable': $this->registerPlugin(self::PLUGIN_MODIFIER, $name, $callback);
                ...
                $this->addDefaultModifiers([$name]);
                break;
            case 'output': $this->BCPluginsAdapter->addCallableAsOutputFilter($callback, $name);
                break;
            case 'pre': $this->BCPluginsAdapter->addCallableAsPreFilter($callback, $name);
                break;
            case 'post': $this->BCPluginsAdapter->addCallableAsPostFilter($callback, $name);
                break;
            default: throw new Exception("Illegal filter type '{$type}'");
        } return $this;
    ...
    } public function unregisterFilter($type, $name)
    {
        if (!is_string($name)) {
            ...
            switch ($type) {
                case 'output': $this->BCPluginsAdapter->removeOutputFilter($name);
                    break;
                case 'pre': $this->BCPluginsAdapter->removePreFilter($name);
                    break;
                case 'post': $this->BCPluginsAdapter->removePostFilter($name);
                    break;
                default: throw new Exception("Illegal filter type '{$type}'");
            }
        } return $this;
    ...
    } public function doCreateTemplate($resource_name, $cache_id = null, $compile_id = null, $parent = null, $caching = null, $cache_lifetime = null, bool $isConfig = false, array $data = []): Template
    {
        ...
        if (!isset($this->templates[$_templateId])) {
            ...
            $newTemplate->templateId = $_templateId;
